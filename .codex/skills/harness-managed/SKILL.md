...
3. `product`、`architecture`、`dependency` Assignment 使用同一输入协议。必须明确返回 `accepted`、`accepted_with_changes`、`deferred` 或 `rejected`；接受的工作必须先通过 `harness requirements intake` 更新或引用本地 Story，向当前用户回读确认并执行 `harness requirements confirm` 转为 `ready` 后才能记录本地 Sprint，Assignment 不得成为第二需求真源。
