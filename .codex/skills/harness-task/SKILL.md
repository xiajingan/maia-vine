...
3. 运行 `harness task-context` 获取 attempt 路径、唯一协议和 `agent_invocations`。`execution_protocol=agent` 时分别创建全新的 Plan/Exec Agent，并让每个 Agent 用自己的 `invocation_id` 和 role 再次调用 `task-context` 完成绑定；禁止复用旧 Agent 或 follow-up turn。计划只写入 `.harness/runs/`。
