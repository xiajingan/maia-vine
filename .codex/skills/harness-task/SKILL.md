...
4. `review_protocol=agent-full` 时独立调用 `harness-review`；重试轮先验证 `open_findings`，全部关闭后才执行最终 Full Review。`artifact-only` 时前台只根据 Action/Gate 的确定性证据生成 Review JSON，不派生 Review Agent。两者都必须写入 `task-context.review_report`，并执行 `task-review` 和 Review Gate。
