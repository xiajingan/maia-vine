...
ACTIONS = {
    ...
    "environment.test.prepare": Action(
        (
            "promote-prep",
            "test",
            "--strict",
        ...
        "Test 部署即时准备检查",
        PREFLIGHT,
        frozenset({"standalone"}),
        "write",
    ),
    "project.promote-test.guard": Action(
        ("promote-test-guard", "--sprint", "{sprint}", "--task-id", "{task_id}"),
        "校验当前 Test 提升 attempt 的不可变部署回执",
        REVIEW,
        frozenset({"standalone"}),
    ),
    "project.integration.guard": Action(
        ("integration-guard", "--sprint", "{sprint}", "--task-id", "{task_id}"),
        "真实环境集成合同与证据检查",
        REVIEW,
        frozenset({"standalone"}),
    ),
    "project.integration.execute": Action(
        ("integration-exec", "--sprint", "{sprint}", "--task-id", "{task_id}"),
        "执行已登记的真实环境集成 producer",
        EXECUTE,
        frozenset({"standalone"}),
        "write",
        1800,
    ...
    "project.acceptance.guard": Action(
        ("acceptance-record", "verify", "{sprint}"),
        "验收记录门禁",
        REVIEW,
        frozenset({"standalone", "managed"}),
    ),
